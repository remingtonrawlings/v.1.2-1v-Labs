...
interface StrategicWorkflowsProps {
  onBack: () => void;
  onNext: (assessments: DiagnosticAssessment[]) => void;
}
...
export const StrategicWorkflows: React.FC<StrategicWorkflowsProps> = ({ onBack, onNext }) => {
    const [assessments, setAssessments] = useState<DiagnosticAssessment[]>(initialScores);

    const updateAssessment = (id: string, updates: Partial<Omit<DiagnosticAssessment, 'id' | 'name' | 'category'>>) => {
        setAssessments(prev => prev.map(a => a.id === id ? { ...a, ...updates } : a));
    }
    
    const sortedPriorities = useMemo(() => {
        const scored = assessments.filter(a => a.maturity !== null);
        const priorityScore = (item: DiagnosticAssessment) => (item.impact * 0.6) + (item.feasibility * 0.4) - (item.maturity * 0.1);
                ...
                <div className="max-w-7xl mx-auto px-6 py-4">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center space-x-4">
                            ...
                            <div>
                                <h1 className="text-2xl font-bold text-gray-900">Step 8: Strategic Workflows</h1>
